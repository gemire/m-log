...
public abstract class AbstractHibernateDao<T> extends HibernateDaoSupport implements BaseDao<T> {
    private static final Logger log = Logger.getLogger(AbstractHibernateDao.class);
    ...
    public Page<T> findPage(Page<T> page, QueryCriterion queryCriterion) {
        ...
        if (page.isAutoCount()) {
            // Query countObject =
            // this.getSession().createQuery(queryCriterion.getCountString());
            // applyQueryCriteriaToQuery(countObject, queryCriterion);
            // page.setTotalCount(new
            // Long(countObject.uniqueResult().toString()));
    ...
    private Query getCountQuery(String queryString, Object[] values) {
        ...
        //
        // countQueryStr = "FROM " + StringUtils.substringAfter(countQueryStr,
        // "FROM");
        // countQueryStr = StringUtils.substringBefore(countQueryStr, "ORDER
        // BY");
        // countQueryStr = StringUtils.substringBefore(countQueryStr, "GROUP
        // BY");
