...
$.extend(mlog.twitter.editor,{
	...
	init : function(conf){
		...
		var div_em_insert = mlog.twitter.editor._get_em_div_id(conf);
		...
		var template = [
			...
			'		<input type="button" class="btn" onclick="mlog.twitter.add();" value="发表" id="btn-posttwitter" />',
			'	</div>',
			'</div>',
		...
		$('#btn_show_em_' + id).click(function(){
			mlog.twitter.editor.showEmInsert(id);
		});
	...
	showEmInsert : function(id){
		...
		var id = mlog.twitter.editor._get_em_div_id({
			target : $('#' + id)
		});
