			...
			<li>
				<div id="comment-${comment.id}" class="comment-body">
					...
					<div class="comment-author">
						<a href="${comment.url?default()}" id="comment-author-${comment.id}" rel="external nofollow" target="_blank">${comment.author}</a>
					...
					<div class="comment-content">
						<#if comment.parent?exists && comment.parent.id != 0>
							...
							<div class="quotewap">
								<div class="quotetop">@${parent.author?default("蒙面大侠")}</div>
								<div class="quotemain">
...
<#if post.commentStatus == "open">
	<form id="comment-form" class="comment-form" target="_self" method="post" action="${base}/comment/post">
		...
		<div class="item">
			作者：<input name="author" type="text" class="text" value="${author?default("")}" style="width:150px;" />&nbsp;&nbsp;
			主页：<input name="url" type="text" class="text" value="${url?default("")}" style="width:150px;" />&nbsp;&nbsp;
			邮箱：<input name="email" type="text" class="text" value="${email?default("")}" style="width:150px;" />&nbsp;&nbsp;
		</div>
		<div class="item">
